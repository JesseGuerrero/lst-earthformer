...
log_and_echo "This assumes caches have been pre-built with setup_data.py"
log_and_echo ""
            ...
            for learning_rate in "${LEARNING_RATES[@]}"; do
                for precision in "${PRECISIONS[@]}"; do
                  for removed_channels in "${REMOVED_CHANNELS[@]}"; do
                      ...
                      log_error "   Removed ChannelsL $removed_channels"
                      log_error "="
                      ...
                      if [ $? -eq 0 ]; then
                          ...
                          log_and_echo "✅ Training Success: cluster=$cluster, input_length=$input_length, max_nodata=$max_nodata, lr=$learning_rate, precision=$precision"
                      else
                          FAILED_RUNS=$((FAILED_RUNS + 1))
                          log_error "❌ Training Failed: cluster=$cluster, input_length=$input_length, max_nodata=$max_nodata, lr=$learning_rate, precision=$precision"
